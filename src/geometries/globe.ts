...
class GlobeGeometry extends BufferGeometry {
  constructor(
    ...
    const heightSegments = Math.ceil((toLat - fromLat) / latStep);
    const widthSegments = Math.ceil((toLng - fromLng) / lngStep);
    ...
    for (let iy = 0; iy <= heightSegments; iy++) {
      ...
      for (let ix = 0; ix <= widthSegments; ix++) {
        ...
        const alt = this.dem?.getHeightAt(lat, lng) ?? 0;

        const u = ix / widthSegments;

        // vertex
        const point = lngLatAltToVector([lng, lat, alt]);
